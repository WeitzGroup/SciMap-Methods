...
NIH_clean$IDC_loss <- NIH_clean$indirect - .15 * NIH_clean$direct

#set negative values to zero; these are cases where there were no indirects before, or the IDC rate was <15% already
NIH_clean$IDC_loss <- ifelse(NIH_clean$IDC_loss < 0, 0, NIH_clean$IDC_loss) 


NIH_clean %>% group_by(org_name) %>% summarize(IDC_loss = sum(IDC_loss, na.rm=TRUE)) %>% arrange(desc(IDC_loss)) %>% head(50) %>% write.csv("top50_IDC.csv", row.names=FALSE)
...
county_commute_NIH[is.na(county_commute_NIH)] <- 0
...
county_commute_NIH_sum <- county_commute_NIH %>%
  ...
  mutate(combined_loss = terminated_loss + IDC_loss - overlap_loss,
         ...
         combined_econ_loss_noself = terminated_econ_loss_noself + IDC_econ_loss - overlap_econ_loss_noself,
         combined_job_loss_noself = terminated_job_loss_noself + IDC_job_loss - overlap_job_loss_noself) %>%
...
#replace all NA values with zeros
county_commute_NIH_inst[is.na(county_commute_NIH_inst)] <- 0

county_commute_NIH_inst <- county_commute_NIH_inst %>%
...
```{r 2022-flows}
commute_raw_22 <- read_csv("data/JT00_mostrecent") %>%
  mutate(DESTINATION = str_pad(w_tract, 11, pad = "0"),
         ORIGIN = str_pad(h_county, 5, pad = "0"))
...
#replace all NA values with zeros
county_commute_NIH_22[is.na(county_commute_NIH_22)] <- 0
...
write.csv(county_commute_NIH_sum_22, "output/NIH_impact_county_most-recent-flows.csv", row.names=FALSE)
```
...
#replace all NA values with zeros
county_commute_NIH_sect[is.na(county_commute_NIH_sect)] <- 0
...
#replace all NA values with zeros
cong_commute_NIH[is.na(cong_commute_NIH)] <- 0
