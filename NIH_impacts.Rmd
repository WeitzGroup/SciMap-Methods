...
missing_fips <- data.frame(org_name =  c("ECOHEALTH ALLIANCE, INC.",
                                         ...
                                         "PRIOGEN CORP"),
                           FIPS = c("36061", "25025", "25017", "17031", "36005", "36059", "27123"),
                           GEOID = c("3612", "2507", "2505", "1701", "3615", "3603", "2704")) %>%
...
missing_coords$lat[missing_coords$org_name=="PRIOGEN CORP"]<-44.970078873271
missing_coords$lon[missing_coords$org_name=="PRIOGEN CORP"]<--93.205053683411
...
```
```{r county-stayvflow}
#how much we assume stays in county - no commuting
county_varystatic <- map_df(seq(0, 1, by = 0.05), function(mult) {
  county_commute_NIH %>%
    mutate(proportion = ifelse(ORIGIN == DESTINATION, mult + (1 - mult) * proportion, (1 - mult) * proportion)) %>%
    mutate(across(starts_with(c("terminated", "IDC", "grant", "overlap")), ~ . * proportion)) %>%
    rename(FIPS = ORIGIN) %>%
    group_by(FIPS) %>% 
    summarise(across(starts_with(c("terminated", "IDC", "grant", "overlap")), ~ sum(.x, na.rm = TRUE))) %>%
...
county_commute_NIH_diffmult <- read.csv("data/county_pop.csv") %>%
                        mutate(FIPS = str_pad(FIPS, 5, pad = "0")) %>%
                        right_join(county_commute_NIH %>% mutate(FIPS = DESTINATION)) %>%
                        left_join(state_mult) %>%
                        mutate(terminated_econ_loss = terminated_loss * econ_mult,
                               IDC_econ_loss = IDC_loss * econ_mult) %>%
                        select(-FIPS)
...
write.csv(diff_mult_county, "output/NIH_impact_county-diffmult.csv", row.names=FALSE)
