...
commute_raw <- fread("./data/OD_tract-to-county_2016.csv") %>% 
...
county_varystatic <- map_df(seq(0, 1, by = 0.05), function(mult) {
  county_commute_NIH %>%
    mutate(proportion = ifelse(ORIGIN == substr(DESTINATION, 1, 5), mult + (1 - mult) * proportion, (1 - mult) * proportion)) %>%
    mutate(across(starts_with(c("terminated", "IDC", "grant", "overlap")), ~ . * proportion)) %>%
    rename(FIPS = ORIGIN) %>%
    group_by(FIPS) %>% 
    summarise(across(starts_with(c("terminated", "IDC", "grant", "overlap")), ~ sum(.x, na.rm = TRUE))) %>%
...
commute_raw_ctc <- fread("./data/OD_tract-to-county_2016.csv") %>% 
  mutate(DESTINATION = str_pad(W_geocode, 11, pad = "0"),
         ORIGIN = str_pad(H_geocode, 5, pad = "0")) %>%
...
```{r 2022-flows}
commute_raw_22 <- fread("./data/home_county_workTR_2022.csv") %>% 
...
commute_raw_sect <- fread("./data/OD_tract-to-county_2016.csv") %>% 
  mutate(DESTINATION = str_pad(W_geocode, 11, pad = "0"),
         ORIGIN = str_pad(H_geocode, 5, pad = "0")) %>%
...
cong_commute <- read.csv("./data/home_119_workTR.csv") %>%
